...
local function update(inst)
 ...
 if show_text then
  ...
  if all_text_shown then
   ...
   -- has been displayed), but
   -- we cannot count it as
   -- finished until the button
   -- is released, otherwise
   -- a button press to skip
   -- the animations would
   -- result in the new game
   -- starting with a dog jump
